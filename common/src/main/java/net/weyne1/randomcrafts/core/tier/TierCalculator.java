...
public class TierCalculator {
    ...
    private static final List<AnchorRule> ANCHOR_RULES = List.of(
            ...
            new AnchorRule("mace", 11),
            new AnchorRule("heavy_core", 11),
            new AnchorRule("iron", 10),
            new AnchorRule("gold", 8),
            new AnchorRule("copper", 6),
    ...
    private static Map<Item, Set<Item>> buildGraph(List<VanillaRecipeData> vanillaRecipes) {
        Map<Item, Set<Item>> graph = new HashMap<>();

        for (VanillaRecipeData vr : vanillaRecipes) {
            graph.put(vr.output(), new HashSet<>(vr.inputs()));
        }
    ...
    private static Map<Item, Integer> initializeBaseTiers(
        ...
        for (Item item : allItems) {
            ...
            // 1. anchor / mining (жёстко фиксированные тиры)
